"Parsing filename metadata and extracting the data from the files"
...
def load_scan_file(filepath: str, amplifier_gain: float = 1e9) -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    meta = filename_metadata(filepath)
    ...
    scan_info = {
        ...
        "x_vec": np.linspace(x_start, x_end, int(x_pixels)),
        "y_vec": np.linspace(y_start, y_end, int(y_pixels)),
        }
    return npdata, scan_info, meta
